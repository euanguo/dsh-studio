...
function updateInfo(version: string, file = 'DSH-Studio-1.2.0-arm64.zip'): UpdateInfo {
  return {
    version,
    files: [{ url: `https://github.com/euanguo/dsh-studio-app/releases/download/v${version}/${file}`, sha512: 'hash', size: 100 }],
    path: file,
    sha512: 'hash',
...
test('official release URLs are fixed to the trusted repository', () => {
  assert.equal(officialReleaseUrl('1.2.3'), 'https://github.com/euanguo/dsh-studio-app/releases/tag/v1.2.3')
...
test('manager offers the official Release page when the platform asset is missing', async () => {
  ...
  if (state.status === 'unsupported') {
    assert.equal(state.releaseUrl, 'https://github.com/euanguo/dsh-studio-app/releases/tag/v1.2.0')
  }
})
