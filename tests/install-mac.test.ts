...
async function makeBundle(path: string, marker: string): Promise<void> {
  ...
  await writeFile(join(path, 'Contents', 'MacOS', 'DSH Studio'), marker)
...
test('local mac install never exposes a partially copied app bundle', async () => {
  const root = await mkdtemp(join(tmpdir(), 'dsh-studio-install-'))
  const source = join(root, 'source.app')
  const destination = join(root, 'Applications', 'DSH Studio.app')
...
async function makeBundleValidation(path: string): Promise<void> {
  const values = await Promise.all([
    readFile(join(path, 'Contents', 'MacOS', 'DSH Studio'), 'utf8'),
    readFile(join(
      path,
      'Contents',
      'Frameworks',
