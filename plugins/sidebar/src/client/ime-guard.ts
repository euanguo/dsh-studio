 ...
 *
 * While a CJK input method is composing (the user is picking a candidate
 * from the IME window), every pressed key BELONGS to the input method:
 * arrows move the candidate highlight, Enter/Space confirm the composition,
 * Escape cancels it. Page code must not process those keys — a component
 * that does (a number stepper calling preventDefault() on ArrowUp/ArrowDown,
 * a submit handler reacting to Enter, ...) silently breaks the IME:
 ...
 * HTML preview's iframe, or an office-suite UI) can never intercept
