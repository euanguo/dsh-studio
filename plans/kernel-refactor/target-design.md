...
> 状态：已定稿（计划阶段产物，revision 2）· 本文档是 `plans/kernel-refactor/` 执行包的架构基准。
> 前序：`.unlazy/deep-refactor/`（findings 收敛轮，ROOT ALL MET）已完成逐条修复；
> **本轮是结构轮**：不再追问题清单，而是把系统收口到下列最终形态。
...
- 宽度记忆（sidebar-service.ts:815-821 按 cwd bucket）与 `layoutScope` 偏好、rail cap
  （clampSidebarWidth）迁入 LayoutService，持久化经 StateStore（3.4）。
...
- **必须修复的结构缺陷**：
  - comments 双写者竞态（diff-comments-store chromeRecord :52 vs review-comments
    chromeRecordComments :138 各持整记录副本回写）→ 收敛为单 slice 单写者；
  - 版本策略二义性：terminal-sessions version!==1 硬拒（terminal-session-store.ts:330）vs
...
- DESKTOP_CHROME_CSS（40-181 约141行内联 CSS 字符串）→ 独立 CSS 模块资产。
