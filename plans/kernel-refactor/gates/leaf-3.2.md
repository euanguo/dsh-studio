...
- [ ] G1: 崩溃窗口 fixture 测试——W1..W5 与 U1..U3 九种磁盘状态各自重建于临时目录，reconcile 结果逐一断言（含致命态 P✗B✗ 的空 profile 重建+error 快照）
  CHECK: bash -lc 'node --test tests/marketplace-reconcile.test.ts && echo RECONCILE-TESTS-OK'
  EXPECT: RECONCILE-TESTS-OK
  CWD: .
  EVIDENCE: pending

- [ ] G2: v1 兼容测试——旧格式读为 phase='applied'/committed=true；下一次成功事务原子升级 v2；升级过程崩溃不留半文件
  CHECK: bash -lc 'node --test --test-name-pattern="v1" tests/marketplace-reconcile.test.ts >/dev/null 2>&1; node --test tests/marketplace-reconcile.test.ts >/dev/null 2>&1 && node -e "console.log('V1-COMPAT-COVERED')" '
  EXPECT: V1-COMPAT-COVERED
  CWD: .
  EVIDENCE: pending

- [ ] G3: 对账副作用收口——removeTree 清理失败在对账路径上显式上报（不再静默 onWarn 吞掉影响判定假设的平台失败）
...
- [ ] G4: 人工确认——journal 预写点位于两个 renameSync 之前（代码走查行号记录）；实机 kill -9 中段演练由 node-3.3 承接
  EVIDENCE: pending
