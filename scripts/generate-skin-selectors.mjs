...
function renderModule(source, anchors) {
  const lines = [
    ...
    `// Sources: ${relative(root, source)} (apps/web/dist/assets + packages/*/*/lib/client.js)`,
    `// DSH revision: ${basename(source)}`,
    '//',
...
function main() {
  ...
  const anchors = resolveAnchors(collectClasses(source))
  const failures = []
  for (const anchor of ANCHORS) {
  ...
  if (checkOnly) {
    ...
    // The pinned DSH build output is not byte-reproducible across machines
    // (class hashes differ per build host), so exact content comparison can
    // only ever pass on the machine that generated the file. Anchor the gate
    // to the DSH revision instead: a revision mismatch means an upstream
    // bump landed without regenerating the selectors, which must fail.
    // Same-revision content drift is machine noise and only warns.
    const committedRevision = committed.match(/^\/\/ DSH revision: ([0-9a-f]+)$/m)?.[1] ?? ''
    const currentRevision = basename(source)
    if (committedRevision !== currentRevision) {
      throw new Error(
        `generated-selectors.ts is stale: pinned DSH revision is ${currentRevision}, `
        + `the committed module records ${committedRevision || '(none)'}; `
    ...
    if (committed !== rendered) {
      ...
      if (drift.stdout || drift.stderr) {
        console.warn(
          'generated-selectors.ts differs from this machine\'s DSH build output '
          + '(class hashes are build-host dependent); the committed module stays authoritative:',
        )
        console.warn(drift.stdout + drift.stderr)
      }
